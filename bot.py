...
async def todo_add(interaction: discord.Interaction, task: str):
    ...
    await interaction.response.send_message(
        embed=make_embed("◉ TASK ADDED", f"**#{count}** · {task}\n\nUse `/todo list` to see all tasks.", color=0x10B981),
        ephemeral=True
    )
...
async def todo_done(interaction: discord.Interaction, number: int):
    ...
    await interaction.response.send_message(
        embed=make_embed(
            "✓ TASK COMPLETE",
            f"**{todos[number-1]['task']}** marked done.\n\n"
...
async def todo_clear(interaction: discord.Interaction):
    ...
    save_data(data)
    await interaction.response.send_message(
        embed=make_embed("◈ LIST CLEARED", "Your task list has been wiped. Fresh start.", color=0x6B6B9A),
        ephemeral=True
    )
