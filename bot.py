...
async def refresh_realtime_leaderboards(guild: discord.Guild = None):
    ...
    if vc_lb_ch:
        ...
        for uid, seconds in vc_time.items():
            ...
            is_live    = gm and gm.voice and gm.voice.channel is not None
            # Add live join-time delta so seconds feel live
            live_extra = int(time_module.time() - _vc_join_times[uid]) if uid in _vc_join_times else 0
            entries.append({
                "uid": uid, "codename": codename,
                "seconds": int(seconds) + live_extra,
                "gm": gm, "live": is_live
            })
